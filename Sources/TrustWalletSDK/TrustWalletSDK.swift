...
public final class TrustWalletSDK {
    ...
    private func handleSignMessage(_ components: URLComponents) -> Bool {
        ...
        let address = components.queryParameterValue(for: "address").flatMap({ MoacAddress(string: $0) })
    ...
    private func handleSignTransaction(_ components: URLComponents) -> Bool {
        ...
        guard let to = components.queryParameterValue(for: "to").flatMap({ MoacAddress(string: $0) }) else {
            return false
        }
        ...
        guard let shardingFlag = components.queryParameterValue(for: "shardingFlag").flatMap({ BigInt($0) }) else {
            return false
        }
        guard let systemContract = components.queryParameterValue(for: "systemContract").flatMap({ BigInt($0) }) else {
            return false
        }
